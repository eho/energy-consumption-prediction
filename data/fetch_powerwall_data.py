...
def fetch_and_save_data(access_token, date, output_dir):
    ...
    """
    start_date = f"{datetime.datetime.combine(date, datetime.time.min).isoformat()}+10:00"
    ...
    try:
        response = requests.get(f"{TESLA_ENDPOINT}/api/1/energy_sites/{ENERGY_SITE_ID}/calendar_history", params=urllib.parse.urlencode(params), headers=headers)
...
@click.command()
@click.option('--start-date', help='Start date (YYYY-MM-DD format)', type=click.DateTime(formats=['%Y-%m-%d']))
